 ...
 * <p>实验二概述：
 * <ol>
 *     <li>{@link MyDate} 的静态方法 {@link MyDate#of(int, int, int)}模拟
 *     {@code LocalDate#of(int, int, int)} 并学习了{@code java.time.Month} 、
 *     {@code java.time.Year} 类。 </li>
 *
 *     <li>实验要求基类 {@link Person} 及派生类 {@link Student}具有成员方法
 *     “开户、存款、取款、查询（余额、明细）、销户”等操作是非常低级的OOP程序设计错误，
 *     折中办法是认为 {@link Person} 持有 {@link Card} 时具有上述一系列动作。
 *     设计其实现时采用了静态工厂模式，遵循“复合优于继承”的原则，抽象产品 {@link Card}
 *     (模拟现实世界中的消费型卡)、具体产品 {@link CampusCardService.CampusCard}
 *     (模拟现实世界的校园一卡通)、具体工厂 {@link CampusCardService} 模拟现实世界中的
 *     校园一卡通服务中心，类的实现模拟了Fork/Join框架的部分设计思想。</li>
 *
 *     <li>基类 {@link Vehicle} 及其子类 {@link Car}、{@link Truck}
 *     巩固了类的继承基础知识。</li>
 *
 *     <li>实验要求" {@link Rectangle} 、{@link Circle}、{@link Triangle}继承 {@link Coordinate} "
 *     不论是用于数学计算还是二维图形描述，提出这一实验要求的人都像是一个OOP初学者，应采用“复合优于继承”
 *     原则解决问题。三个类实现了{@link Shape} 以及数据合法性验证，{@link Triangle}三边关系验证。</li>
 *
 *     <li>{@link YMD_2}、{@link 实验二.content.Mypackage.Test_YMD} 按实验要求将源代码粘贴并运行。</li>
 * </ol></p>
 * @author 段云飞
 * @since 2019-10-24
 */
public final class ExpTwo {
    ...
    public static void expTwo_2_Verify(){
        ...
        try {
            ...
            //这里并没有想出余额不足时的处理办法
            //了能是设计模式方面的知识有待提高
            while (true) Mr_Duan.drawMoney(998);
